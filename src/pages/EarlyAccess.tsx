...
const EarlyAccess = () => {
  ...
  useEffect(() => {
    ...
    const fetchCount = async () => {
      const { count } = await supabase.from("early_access_signups").select("*", { count: "exact", head: true });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative pt-32 pb-24 md:pt-40 md:pb-32 overflow-hidden">
        ...
        <div className="container relative z-10">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-black leading-[1.1] mb-6">
              Get <span className="text-gradient-warm">Early Access</span>
            </h1>

            <p className="text-lg text-muted-foreground max-w-xl mx-auto mb-10 leading-relaxed">
              Join the waitlist and be among the first PMs to transform from ticket manager to product strategist. Plus,
              get instant access to our <span className="text-foreground font-medium">free PRD Review Analysis</span>.
            </p>
          ...
          >
            <div className="rounded-xl border border-border bg-card p-6 md:p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed mb-4">
                Sign up for early access and get an AI-powered analysis of your PRD—highlighting gaps, vague
                requirements, and missing success metrics before your team writes a single line of code.
              </p>
              <a
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 sm:gap-0">
              {roadmapItems.map((item, i) => (
                <div key={item.phase} className="flex items-center gap-4 sm:flex-col sm:gap-3 sm:text-center flex-1">
                  ...
                  {i < roadmapItems.length - 1 && <div className="hidden sm:block flex-1 h-px bg-border mx-4" />}
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <div
          className="absolute bottom-1/4 right-8 w-48 h-48 rounded-full bg-accent/5 blur-3xl animate-float"
